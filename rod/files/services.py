...
from config.s3_client import s3_generate_presigned_post
from rod.files.enums import FileUploadStorage
from rod.files.models import File
from rod.files.utils import bytes_to_mib
from rod.files.utils import file_generate_local_upload_url
from rod.files.utils import file_generate_name
from rod.files.utils import file_generate_upload_path
...
def _validate_file_size(file_obj):
    ...
    if file_obj.size > max_size:
        msg = f"File is too large. It should not exceed {bytes_to_mib(max_size)} MiB"
...
class FileDirectUploadService:
    """
    This also serves as an example of a service class,
    which encapsulates a flow (start & finish) + one-off action
    (upload_local) into a namespace.
