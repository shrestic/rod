...
from rod.common.types import DjangoModelType
...
def model_update(
    *,
    ...
    Some important notes:
        ...
        - There's a strict assertion that all values in `fields` are actual fields
          in `instance`.
        - `fields` can support m2m fields, which are handled after the update
          on `instance`.
        - If `auto_updated_at` is True, we'll try bumping `updated_at`
